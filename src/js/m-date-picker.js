(function($){
	...
	**/
	...
	mdatepicker.prototype.show = function(){
		...
		setTimeout(function(){
			this.obj.eq(1).addClass("md-active");
		}.bind(this),0);
		if(ops.mode==='date'){
			initYear(year)
			initMonth(month)
	...
	mdatepicker.prototype.hide = function(){
		var self = this;
		$(self.obj).eq(0).remove();
		$(self.obj).eq(1).removeClass("md-active")
		setTimeout(function(){
			$(self.obj).remove();
		},300)
	...
	function getDatepickerHtml(ops,text){
		...
		unitHtml = ops.unitVisiable?unitHtml:'';
		html = 
		'<div class="md-overlayer" ontouchend="$.mdatepickerHandle.handleCancel()"></div><div class="md-wrap">' +
			headHtml +
			unitHtml +
			bodyHtml +
		'</div>';
